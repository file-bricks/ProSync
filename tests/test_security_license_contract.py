"""Automated security, dependency floor, and third-party license contract tests for ProSync."""
...
def test_dependency_vulnerability_floors() -> None:
    ...
    assert re.search(r"^paramiko\s*>=\s*3\.4\.0", req_text, re.MULTILINE), (
        "requirements.txt must enforce paramiko>=3.4.0 floor (mitigating Terrapin GHSA-45x7-px36-x8w8)"
    )
    assert re.search(r"^pypdf\s*>=\s*4\.0\.0", req_text, re.MULTILINE), (
        "requirements.txt must enforce pypdf>=4.0.0 floor"
    )
    ...
    assert "paramiko>=3.4.0" in pyproject_text, "pyproject.toml must specify paramiko>=3.4.0"
    assert "pypdf>=4.0.0" in pyproject_text, "pyproject.toml must specify pypdf>=4.0.0"
...
def test_security_policy_bilingual_and_sla() -> None:
    ...
    # SLA commitment
    assert "48 Stunden" in sec_text or "48-Stunden" in sec_text or "48 Stunden" in sec_text, "SECURITY.md must define 48h SLA in German"
    assert "48 hours" in sec_text or "48-hour" in sec_text, "SECURITY.md must define 48h SLA in English"
...
def test_local_first_and_offline_invariants() -> None:
    ...
    core_files = [
        ROOT / "ProSyncStart_V3.1.py",
        ROOT / "ProSyncReader.py",
        ROOT / "logger.py",
        ROOT / "translator.py",
        ROOT / "cross_os_rules.py",
        ROOT / "schedule_time.py",
    ...
    for source_file in core_files:
        ...
        for pat in disallowed_patterns:
            assert not pat.search(text), f"Found disallowed telemetry pattern {pat.pattern} in {source_file.name}"
